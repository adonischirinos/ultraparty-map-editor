...
mparse_LoadMap(map[])
{
    ...
    {
        new start_index;
        if((start_index = strfind(buffer, "SetDynamicObjectMaterialText")) != -1)
        {
            new
            ...
            if(
                sscanf(buffer[start_index], "p<(>{s[30]}p<,>s[31]is[50]is[50]iixxp<)>i", prefix, materialindex, text, materialsize, fontface, fontsize, bold, fontcolor, backcolor, alignment) &&
                sscanf(buffer[start_index], "p<(>{s[30]}p<,>s[31]is[50]is[50]iiiip<)>i", prefix, materialindex, text, materialsize, fontface, fontsize, bold, fontcolor, backcolor, alignment)
            ){
                continue;
            }
        ...
        else if((start_index = strfind(buffer, "SetDynamicObjectMaterial")) != -1)
        {
            ...
            if(
                sscanf(buffer[start_index], "p<(>{s[25]}p<,>s[31]iis[50]s[50]p<)>x", prefix, materialindex, modelid, txd, texture, color) &&
                sscanf(buffer[start_index], "p<(>{s[25]}p<,>s[31]iis[50]s[50]p<)>i", prefix, materialindex, modelid, txd, texture, color)
            ){
                continue;
            }
        ...
        else if((start_index = strfind(buffer, "CreateDynamicObject")) != -1)
        {
            ...
            if(start_index > 0 && !sscanf(buffer, "p<=>s[31]p<(>{s[21]}p<,>ifffffp<)>f", prefix, modelid, x, y, z, rx, ry, rz))
                prefix_found = true, stream_distance = 0.0;
            else if(start_index > 0 && !sscanf(buffer, "p<=>s[31]p<(>{s[21]}p<,>iffffffp<)>f", prefix, modelid, x, y, z, rx, ry, rz, stream_distance))
                prefix_found = true;
            else if(!sscanf(buffer[start_index], "p<(>{s[20]}p<,>ifffffp<)>f", modelid, x, y, z, rx, ry, rz))
                prefix_found = false, stream_distance = 0.0;
            else if(!sscanf(buffer[start_index], "p<(>{s[20]}p<,>iffffffp<)>f", modelid, x, y, z, rx, ry, rz, stream_distance))
                prefix_found = false;
            else
                continue;
